...
const Navbar = ({ onSearch }) => {
  ...
  const searchEnabledPaths = ["/", "/home", "/categories", "/new-releases", "/liked"];
    ...
    <nav className="bg-gray-900 text-white p-4 flex justify-between items-center fixed top-0 left-0 w-full z-50">
      <div className="flex items-center">
        <img
          ...
          style={{ maxWidth: "40px" }}
        />
        <span className="hidden sm:inline text-2xl font-bold text-green-600 tracking-wide">
          Spotify
        </span>
      </div>
      ...
      {isSearchEnabled ? (
        <form onSubmit={submit} className="flex items-center xl:mr-40 flex-1 max-w-3xl mx-8">
          <input
